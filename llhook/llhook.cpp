...
namespace
{
	...
	public:
		...
		bool operator ==(KeyInfo& key_info)
		{
			return v_code == (key_info.v_code &&
				ctrl_flag == key_info.ctrl_flag &&
				shift_flag == key_info.shift_flag &&
				alt_flag == key_info.alt_flag);
		}
	};
	...
	bool IsInterestedKeyEvent(KBDLLHOOKSTRUCT* pLLKeyInfo);
	bool IsInterestedMouseButton(DWORD vkCode);
	bool IsInterestedGesture(DWORD giGesture);

	std::vector<KeyInfo> WantedKeys_;
...
extern "C" bool _declspec (dllexport) InstallKeyboardHook(HWND hWindow)
{
	...
	if(::IsWindow(hWindow))
	{
		...
		KeyInfo key_info = { };
		key_info.v_code = VK_LWIN;
		key_info.ctrl_flag = true;
		key_info.alt_flag = false;
		key_info.shift_flag = false;

		WantedKeys_.push_back(key_info);
...
namespace
{
	LRESULT CALLBACK LowLevelKeyboardProc(_In_  int nCode,  _In_  WPARAM wParam,  _In_  LPARAM lParam)
	{
		if(nCode == HC_ACTION)
		{
			...
			if(IsInterestedKeyEvent(pKeyInfo))
			{
				::PostMessage(hKeyboardClinet_, MSG_COMMAND_1, (WPARAM)(0), (LPARAM)0);
				return 1;
			}
	...
	bool IsInterestedKeyEvent(KBDLLHOOKSTRUCT* pLLKeyInfo)
	{
		...
		key_info.v_code = pLLKeyInfo->vkCode;
		key_info.ctrl_flag = (GetAsyncKeyState(VK_CONTROL) & 0x8000);
		key_info.shift_flag = (GetAsyncKeyState(VK_SHIFT) & 0x8000);
		key_info.alt_flag = (GetAsyncKeyState(VK_MENU) & 0x8000);

		for (int i = 0; i < WantedKeys_.size(); i++)
		{
			if (WantedKeys_[i].v_code == key_info.v_code &&
				WantedKeys_[i].alt_flag == key_info.alt_flag &&
				WantedKeys_[i].shift_flag == key_info.shift_flag &&
				WantedKeys_[i].ctrl_flag == key_info.ctrl_flag)
			{
				return true;
			}
		}
